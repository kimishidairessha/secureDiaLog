          ...
          child: DataTable(
            columns: [
              ...
              ...List.generate(
                  maxTooltipStrength,
                  (index) =>
                      DataColumn(label: Text('Updating(Time) ${index + 1}'))),
              const DataColumn(label: Text('Time')),
            ...
            rows: List.generate(maxRows, (rowIndex) => rowIndex)
                .where((rowIndex) =>
                    widget.strengthTimeList[rowIndex] !=
                    'none') // Filter out rows where timeList is '0'
                .map((rowIndex) => DataRow(cells: [
                      DataCell(Text(widget.timeList[rowIndex])),
                      DataCell(Text(widget.strengthTimeList[rowIndex])),
                      DataCell(
                          Text(mapValueToText(widget.strengthList[rowIndex]))),
                      ...List.generate(maxTooltipStrength, (index) {
                        if (index <
                            widget.strengthToolTipsList[rowIndex].length) {
                          return DataCell(Text(
                              '${mapValueToText(widget.strengthToolTipsList[rowIndex][index].val)} (${TimeUtils.convertHHmmToClock(widget.strengthToolTipsList[rowIndex][index].time)})'));
                        } else {
                          return const DataCell(Text(
                              '')); // Empty cell for rows with fewer tooltips
                        }
                      }),
                      ...
                      ...List.generate(maxTooltipFasting, (index) {
                        if (index <
                            widget.fastingToolTipsList[rowIndex].length) {
                          return DataCell(Text(
                              '${setNull(widget.fastingToolTipsList[rowIndex][index].val)} (${TimeUtils.convertHHmmToClock(widget.fastingToolTipsList[rowIndex][index].time)})'));
                        } else {
                          return const DataCell(Text(
                              '')); // Empty cell for rows with fewer tooltips
                        }
                      ...
                      ...List.generate(maxTooltipPostprandial, (index) {
                        if (index <
                            widget.postprandialToolTipsList[rowIndex].length) {
                          return DataCell(Text(
                              '${setNull(widget.postprandialToolTipsList[rowIndex][index].val)} (${TimeUtils.convertHHmmToClock(widget.postprandialToolTipsList[rowIndex][index].time)})'));
                        } else {
                          return const DataCell(Text(
                              '')); // Empty cell for rows with fewer tooltips
                        }
                      ...
                      ...List.generate(maxTooltipSystolic, (index) {
                        if (index <
                            widget.systolicToolTipsList[rowIndex].length) {
                          return DataCell(Text(
                              '${setNull(widget.systolicToolTipsList[rowIndex][index].val)} (${TimeUtils.convertHHmmToClock(widget.systolicToolTipsList[rowIndex][index].time)})'));
                        } else {
                          return const DataCell(Text(
                              '')); // Empty cell for rows with fewer tooltips
                        }
                      ...
                      ...List.generate(maxTooltipDiastolic, (index) {
                        if (index <
                            widget.diastolicToolTipsList[rowIndex].length) {
                          return DataCell(Text(
                              '${setNull(widget.diastolicToolTipsList[rowIndex][index].val)} (${TimeUtils.convertHHmmToClock(widget.diastolicToolTipsList[rowIndex][index].time)})'));
                        } else {
                          return const DataCell(Text(
                              '')); // Empty cell for rows with fewer tooltips
                        }
                      ...
                      ...List.generate(maxTooltipWeight, (index) {
                        if (index <
                            widget.weightToolTipsList[rowIndex].length) {
                          return DataCell(Text(
                              '${setNull(widget.weightToolTipsList[rowIndex][index].val)} (${TimeUtils.convertHHmmToClock(widget.weightToolTipsList[rowIndex][index].time)})'));
                        } else {
                          return const DataCell(Text(
                              '')); // Empty cell for rows with fewer tooltips
                        }
                      ...
                      ...List.generate(maxTooltipHeartRate, (index) {
                        if (index <
                            widget.heartRateToolTipsList[rowIndex].length) {
                          return DataCell(Text(
                              '${setNull(widget.heartRateToolTipsList[rowIndex][index].val)} (${TimeUtils.convertHHmmToClock(widget.heartRateToolTipsList[rowIndex][index].time)})'));
                        } else {
                          return const DataCell(Text(
                              '')); // Empty cell for rows with fewer tooltips
                        }
